...
class FemaleCreator{
  ...
  public:
    FemaleCreator(){
       ...
       subPrinter = n.subscribe("pritnVec", 1000, &FemaleCreator::chatterCallBackPrint, this);
    }
...
void FemaleCreator::chatterCallBackPrint(const animal_msgs::Int16& receivedMsg){
  for(vector<Dog>::iterator it=femaleDogsVec.begin(); it!=femaleDogsVec.end(); it++){
    it->print();
  }
}
