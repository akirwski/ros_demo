...
class Breeder {
    ...
    Breeder(string subTopicOne, string subTopicTwo, string pubTopic){
        ...
        subPrinter = n.subscribe("pritnVec", 1000, &Breeder::chatterCallBackPrint, this);
    }
...
void Breeder::chatterCallBackPrint(const animal_msgs::Int16& receivedMsg){
  for(vector<Dog>::iterator it=maleVec.begin(); it!=maleVec.end(); it++){
   it->print();
  }

  for(vector<Dog>::iterator it=femaleVec.begin(); it!=femaleVec.end(); it++){
    it->print();
  }
}
